...
**You don't have to do any of this manually.** Type `start` in your agent and it walks you through setup step by step, does most of it for you, and verifies every tool actually works. This page is the reference behind that flow.
...
One more thing to expect: now and then your agent needs a small helper tool to do a job (a Python package to read a spreadsheet export, a scraper's browser). It will always say what it is installing and why before it does, and it never installs anything silently.

## Core (set these up to run the fundamentals)

- **Web research - nothing to set up.** Your coding agent searches the web and reads pages out of the box. That covers most of what market research needs: review pages, forums, articles, competitor sites.
- **Google Workspace (Docs & Sheets)** - lets your agent read and write your Google Docs and Sheets: share a research doc or learnings summary with a coach or client as a Doc, read briefs people send you, pull numbers from a Sheet (like an Ads Manager export). Connect it in your agent's integration settings - in Claude Code that is claude.ai → Settings → Connectors → Google Drive; other agents wire their own integrations (ask yours what it supports). One quirk to know in Claude Code: it can create Docs from plain text, but cannot edit or delete existing Drive files.
- **The video tools (yt-dlp + ffmpeg + Gemini)** - together these power `/watch-video`: drop any ad or reel and your agent actually watches it - frames, transcript, pacing numbers, and how it feels. Used for tearing down competitor winners before imitating them, and for studying your own ads when pulling learnings.
  ...
  3. Store it in your shell environment as `GEMINI_API_KEY` (ask your agent to do this for you).
...
The last command downloads its stealth browser (a few hundred MB, one time). After that, your agent drives it directly whenever research hits a wall - on a Mac it runs it through `~/.cs-tools/bin/python`. One thing to know: plain HTTP mode gets blocked by Reddit - the stealth mode (`StealthyFetcher`) is the one that works, and it needs that browser download.
...
3. Store it in your shell environment as `APIFY_TOKEN` (ask your agent to do this for you - it knows where it goes on your system).

Your agent then runs scrapes through Apify's actors (pre-built scrapers in their store) - for example, an Instagram profile scrape to find posts, then a comment scrape on those posts. Start on the free tier; going deeper is not expensive - if the research matters, pay to win.
...
- **Automated weekly pulses** - by default, the workspace nudges you to run `/research-pulse` when a brand's last sweep is 7+ days old, and you run it yourself. If you want true automation (the sweep running on a schedule without you), ask your agent about setting up a scheduled run - it is a power-user move, and the nudge covers most people better.
